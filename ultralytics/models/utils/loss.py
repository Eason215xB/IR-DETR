...
class DETRLoss(nn.Module):
    ...
    def __init__(self,
        ...
        self.wiou_loss = WiseIouLoss(ltype='ShapeIoU', monotonous=False, inner_iou=False)
    ...
    def _get_loss_bbox(self, pred_bboxes, gt_bboxes, postfix=''):
        ...
        if self.use_wiseiou:
            # loss[name_giou] = self.wiou_loss(pred_bboxes, gt_bboxes, ret_iou=False, ratio=0.7)
            loss[name_giou] = self.wiou_loss(pred_bboxes, gt_bboxes, ret_iou=False, ratio=0.7, **{'scale': 2.7})  # Wise-ShapeIoU,Wise-Inner-ShapeIoU
            # loss[name_giou] = self.wiou_loss(pred_bboxes, gt_bboxes, ret_iou=False, ratio=0.7, **{'mpdiou_hw':2}) # Wise-MPDIoU,Wise-Inner-MPDIoU
        else:
